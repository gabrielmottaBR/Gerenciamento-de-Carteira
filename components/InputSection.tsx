...
const InputSection: React.FC<InputSectionProps> = ({ onSimulate, isLoading }) => {
  ...
  return (
    <div className="w-full flex flex-col lg:flex-row gap-6">
        ...
        {/* Left Panel: Inputs */}
        <div className="w-full lg:w-5/12 bg-[#151b2b] rounded-xl border border-slate-800 shadow-lg flex flex-col shrink-0">
          
          {/* Mode Toggle Compact */}
          <div className="px-5 py-3 border-b border-slate-800 bg-[#0f1522] rounded-t-xl">
             <div className="bg-[#1e293b] p-1 rounded-lg flex text-sm font-bold">
                ...
                >
                   <Briefcase size={16} /> <span className="hidden sm:inline">Novo Investimento</span><span className="sm:hidden">Novo</span>
                </button>
                <button 
          ...
          <div className="p-5">
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  {!showMarketSelector ? (
                    ...
                    >
                      <Sparkles size={18} className="text-amber-400 group-hover:scale-110 transition-transform shrink-0"/>
                      <div className="text-left overflow-hidden">
                         <div className="text-sm font-bold truncate">Sugestão IA</div>
                         <div className="text-[10px] text-amber-400/60 font-medium truncate">Top 10 Eficientes</div>
                      </div>
                    </button>
                  ) : (
                  ...
                  {!showMarketSelector && (
                    ...
                    >
                      <Wand2 size={18} className="text-purple-400 group-hover:rotate-12 transition-transform shrink-0"/>
                      <div className="text-left overflow-hidden">
                         <div className="text-sm font-bold truncate">Prever (FF5)</div>
                         <div className="text-[10px] text-purple-400/60 font-medium truncate">Modelo Fatorial</div>
                      </div>
                    </button>
                  )}
                </div>
              ...
              {/* Assets List */}
              <div className="flex flex-col border border-slate-800 rounded-lg bg-[#0b0f19]/50">
                <div className="flex justify-between items-center p-3 border-b border-slate-800 bg-[#0b0f19] rounded-t-lg">
                ...
                <div className="p-3 space-y-2 max-h-[400px] overflow-y-auto custom-scrollbar">
                  {assets.map((asset, index) => (
                    <div key={index} className="grid grid-cols-12 gap-2 items-center bg-[#0b0f19] p-2 rounded border border-slate-800 hover:border-slate-700 group transition-colors">
                      ...
                      <div className="col-span-5 flex items-center justify-end gap-1 relative">
                         <TrendingUp size={14} className={`absolute left-0 hidden sm:block ${asset.expectedMonthlyReturn >= 0 ? 'text-emerald-500/50' : 'text-rose-500/50'}`} />
                      ...
                      <div className="col-span-2 flex justify-end">
                        <button type="button" onClick={() => handleRemoveAsset(index)} className="text-slate-600 hover:text-rose-500 p-1.5 rounded transition-opacity"><Trash2 size={16} /></button>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div>
                 ...
                 <div className="grid grid-cols-3 gap-3">
                    {['MIN_RISK', 'MAX_SHARPE', 'MAX_RETURN'].map((s) => (
                        <label key={s} className={`cursor-pointer border rounded-lg p-2 md:p-3 flex flex-col items-center justify-center gap-1.5 transition-all ${settings.strategy === s ? (s === 'MIN_RISK' ? 'bg-blue-900/20 border-blue-500/50' : s === 'MAX_SHARPE' ? 'bg-emerald-900/20 border-emerald-500/50' : 'bg-purple-900/20 border-purple-500/50') : 'bg-[#0b0f19] border-slate-700 hover:border-slate-600'}`}>
                            ...
                            <span className={`text-[10px] sm:text-xs font-bold text-center uppercase whitespace-nowrap ${settings.strategy === s ? 'text-white' : 'text-slate-500'}`}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    {settings.mode === 'INVESTMENT' ? (
                        <div className="bg-[#0f1522] border border-slate-700 rounded-lg px-3 flex items-center h-[50px] text-xs sm:text-sm text-slate-300 leading-tight">
                           <span className="text-emerald-400 font-bold mr-2">Hoje:</span> <span className="truncate">Preços de fechamento.</span>
                        </div>
                    ) : (
        ...
        {/* Right Panel: Tutorial & Concepts (Fluid height - No internal scroll lock) */}
        <div className="w-full lg:w-7/12 flex flex-col">
           <div className="bg-[#151b2b] border border-slate-800 rounded-xl flex-1 flex flex-col shadow-lg">
              
              <div className="flex items-center gap-4 p-6 border-b border-slate-700 shrink-0">
                  <div className="p-3 bg-emerald-500/10 rounded-lg text-emerald-400">
                     <BookOpen size={28} />
                  </div>
                  <div>
                     <h2 className="text-xl sm:text-2xl font-bold text-slate-100">Guia & Conceitos</h2>
                     <p className="text-xs sm:text-sm text-slate-400 uppercase tracking-wider">Teoria Moderna de Portfólios (Markowitz)</p>
                  </div>
              </div>

              {/* Content area that grows naturally with page */}
              <div className="p-6 space-y-6 lg:space-y-8">
                    ...
                    <div className="grid grid-cols-1 gap-5">
                        <div className="bg-[#0b0f19] p-5 rounded-lg border border-slate-800">
                            ...
                            <p className="text-sm text-slate-400 leading-relaxed">
                                É o conjunto de carteiras que oferecem o <strong>maior retorno esperado</strong> para um nível definido de risco. Qualquer ponto abaixo da curva é considerado "ineficiente".
                            </p>
                        </div>
                        <div className="bg-[#0b0f19] p-5 rounded-lg border border-slate-800">
                            ...
                            <p className="text-sm text-slate-400 leading-relaxed">
                                Mede o "custo-benefício" do investimento. Calcula quanto retorno excedente você recebe para cada unidade de risco (volatilidade) que assume.
                            </p>
                        </div>
                    </div>
                </section>
                ...
                <section>
                    ...
                    <div className="space-y-5">
                        <div className="flex gap-4 sm:gap-5 bg-[#0f1522] p-4 rounded-lg border border-slate-800/50">
                            <div className="flex-shrink-0 w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center font-bold text-sm sm:text-base text-emerald-400">1</div>
                            <div>
                                <h4 className="font-bold text-slate-200 text-sm sm:text-base">Defina os Ativos e Expectativas</h4>
                                <p className="text-xs sm:text-sm text-slate-400 mt-1 leading-relaxed">
                                Insira os códigos das ações. O sistema busca a volatilidade histórica, mas você deve inserir o <strong>Retorno Mensal Esperado</strong> ou usar a função "FF5".
                                </p>
                            </div>
                        </div>

                        <div className="flex gap-4 sm:gap-5 bg-[#0f1522] p-4 rounded-lg border border-slate-800/50">
                            <div className="flex-shrink-0 w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center font-bold text-sm sm:text-base text-emerald-400">2</div>
                            <div>
                                <h4 className="font-bold text-slate-200 text-sm sm:text-base">Escolha sua Estratégia</h4>
                                <p className="text-xs sm:text-sm text-slate-400 mt-1 leading-relaxed">
                        ...
                        <div className="flex gap-4 sm:gap-5 bg-[#0f1522] p-4 rounded-lg border border-slate-800/50">
                            <div className="flex-shrink-0 w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center font-bold text-sm sm:text-base text-emerald-400">3</div>
                            <div>
                                <h4 className="font-bold text-slate-200 text-sm sm:text-base">Backtesting (Prova Real)</h4>
                                <p className="text-xs sm:text-sm text-slate-400 mt-1 leading-relaxed">
                                Simule a compra da carteira em uma data passada e verifique o resultado financeiro.
                                </p>
                            </div>
                        </div>
                    </div>
